...
// set_time_limit(5); // ограничение выполнения скрипта
$n = 0;
    ...
    define ("SITE", "https://medel-it.com.ua/");
    ...
    define ("FREQUENCY", "weekly"); // позже переопределил ПЕРЕМЕННЫМИ
    ...
    define ("PRIORITY", "0.5");    // позже переопределил ПЕРЕМЕННЫМИ
...
function SkipURL ($url)
{
    ...
    if (isset ($skip_url))
    {
        ...
        {           
			...
			// FIX - для ссылок закрытых слешем вконце
			$v = str_replace('//',     '/',         $v); // ADED
			$v = str_replace('https:/','https://',  $v); // ADED
			$v = str_replace('http:/' ,'http://',   $v); // ADED
			
			$v = str_replace('/:443', ':443', $v);       // ADED - неподписанный сертификат
			...
			if (substr_count($url, $v) > 0)   {
														...
														//ЛОГИРОВАНИЕ:
														//$var = 'YES url: ' . $url . ' v = ' .$v;
														//file_put_contents("DEBUG_log_4.txt", $var . "\n\r", FILE_APPEND);				
				return true; // Skip this URL - ADED - FIXED
...
function Scan ($url)
{
    ...
    foreach ($a1 as $next_url)
    {
        ...
        if (Scan ($next_url))
        {
					   ...
					   // просто FIX тк Главную выдавал так:   https://medel-it.com.ua// 
					   $next_url = str_replace('//','/',$next_url);
					   $next_url = str_replace('https:/','https://',$next_url);
					   $next_url = str_replace('http:/', 'http://' ,$next_url);
			
			
					// -------------------------------- ADED: -----------------------------------------------------------------------------------------------------
					$Kolichestvo_sleshey_in_URL = substr_count(htmlentities ($next_url),"/");   // подсчет кол-ва слешей

					if  ($Kolichestvo_sleshey_in_URL == 3){
						// Главная - 3 слеша
						$PRIORITY  = 1;
						$FREQUENCY = 'always';
					}

					if  ($Kolichestvo_sleshey_in_URL == 4){
						// Категории:  https://medel-it.com.ua/clinic/ - 4 слеша
						$PRIORITY  = 0.9;
						$FREQUENCY = 'daily';
					}
